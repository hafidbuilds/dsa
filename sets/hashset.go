...
func (s *HashSet[E]) String() string {
	...
	sb.WriteByte('{')
	for i, entry := range sequence.Enum(s.Iter) {
		if i > 0 {
			sb.WriteByte(' ')
		}
		if _, err := fmt.Fprint(&sb, entry); err != nil {
			panic(fmt.Errorf("hashset: write element at index %d: %w", i, err))
		}
	}
	sb.WriteByte('}')
	return sb.String()
...
func (s *HashSet[E]) Iter(yield func(E) bool) {
	...
	s.backend.Iter(func(e *hashmap.Entry[E, none]) bool {
		return yield(e.Key())
	})
}
